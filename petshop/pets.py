...
def search(field, value):
    ...
    if order == "asc":
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tags_pets tp where tp.tag = (select t.id from tag t where name = ?) and p.id = tp.pet and p.species = s.id order by p.{oby}",[value])
    else:
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tags_pets tp where tp.tag = (select t.id from tag t where name = ?) and p.id = tp.pet and p.species = s.id order by p.{oby} desc",[value])
